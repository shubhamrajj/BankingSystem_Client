...
function ViewDetails() {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:51996/api/GetAllProfiles",
        ...
        success: function (data) {
            ...
            $.each(data, function (i, item) {
                ...
                var row = "<tr><td>" + item.AccountNo + "</td> <td> " + item.CustomerName + "</td><td>" + item.PhoneNo + "</td><td>" + item.Addrss + "</td><td>" + item.MailId + "</td><td>" + item.DtOfOpeningAccnt + "</td><td>" + item.AccountType + "</td><td>" + item.Balance + "</td><td><button id='b1' OnClick='OpenUpdateModel(" + item.AccountNo + ")'>Edit</button></td><td><button id='b2' OnClick='Delete(" + item.AccountNo + ")'>Delete</button></td></tr > ";
                $('#tbody1').append(row);
...
function Delete(rid) {
    ...
    if (ans) {
        ...
        $.ajax({
            url: "http://localhost:53682/api/DeleteCustomer",
            type: "Delete",
            contentType: "application/json;charset=UTF-8",
            dataType: "json",
            success: function (result) {
